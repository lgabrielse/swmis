...
$query_PatRecpt = "SELECT r.id, r.amt, DATE_FORMAT(r.entrydt,'%d-%b-%Y') entrydt, r.entryby, pp.medrecnum, pp.lastname, pp.firstname, pp.othername, pp.hospital FROM `receipts` r join `patperm` pp on r.medrecnum = pp.medrecnum WHERE r.id = '" . $colname_rid."'";
...
mysql_select_db($database_swmisconn, $swmisconn);
$query_OrdRecpt = "SELECT f.id, left(f.dept,3) dept, f.section, f.name, o.id ordid, o.amtpaid from orders o join fee f on o.feeid = f.id join receipts r on (instr(r.ordlist,o.id)> 0) WHERE r.id = '" . $colname_rid."'";
...
<?php  }?>
  ...
  <tr>
    <td colspan="2" align="center" class="Black_24"><?php echo $row_PatRecpt['lastname'] ?>,<?php echo $row_PatRecpt['firstname'] ?> (<?php echo $row_PatRecpt['othername'] ?>)</td>
  </tr>
  <tr>
