...
if($_POST['pat_type'] == 'InPatient'){
  $updateSQL = sprintf("UPDATE orders SET urgency=%s, doctor=%s, entryby=%s, entrydt=%s, comments=%s WHERE id=%s",
...
<?php if($row_editdel['pat_type'] == "InPatient") { ?>
	    <tr>
		  ...
		  <td nowrap="nowrap" bgcolor="#BCFACC">
		  <input name="nunits" type="text" id="nunits" value="<?php echo $row_editdel['nunits']; ?>" size="3" maxlength="3"  disabled="disabled" />
<?php if(strpos(' Tablet(s) Capsules(s)', $row_editdel['unit']) > 0) { ?>
		    <select name="unit" id="unit" disabled="disabled" > 
			  ...
			  <input name="every" type="hidden" id="every" value="" disabled="disabled" />
...
<?php } ?>
		    for
            <input name="fornum" type="text" id="fornum" value="<?php echo $row_editdel['fornum']; ?>" size="3"  disabled="disabled" />
