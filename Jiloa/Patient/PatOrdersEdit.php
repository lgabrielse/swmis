...
mysql_select_db($database_swmisconn, $swmisconn);
$query_ordedit = sprintf("Select o.id ordid, o.medrecnum, o.visitid, o.feeid, o.doctor, o.status, o.urgency, substr(o.urgency,1,1) urg, DATE_FORMAT(o.entrydt,'%%d-%%b-%%Y %%H:%%i') entrydt, o.entryby, o.amtdue, o.rate, o.ratereason, o.amtpaid, o.comments,  f.dept, f.section, f.name, f.descr, f.fee, f.id feeid FROM orders o join fee f on o.feeid = f.id WHERE o.id = %s", $colname_ordedit);
